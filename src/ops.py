...
def chunk_flip_join(input: torch.Tensor, dim, op):
    input_fwd, input_bwd = input.chunk(2, dim=dim)
    input_bwd = torch.flip(input_bwd, dims=(dim,))
    if op == "sum":
...
def dwconv(input: torch.Tensor, weight, bias):
    return F.conv1d(input.mT, weight, bias, padding="same", groups=input.shape[-1]).mT


def ssm_params(xBC: torch.Tensor, D_weight, D_bias, d_inner, headdim, ngroups):
...
class HydraSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    @staticmethod
    @torch.amp.custom_fwd(device_type="cuda" if torch.cuda.is_available() else "cpu")
    def forward(
        ctx,
        zxBCdt: torch.Tensor,
        ...
        # Check some shapes
        d_xBC = d_inner + 2 * (2 * n_groups * d_state)
        d_conv = conv1d_weight.shape[2]
        assert d_conv % 2 == 1

        assert zxBCdt.shape == (batch, seqlen, d_inner + d_xBC + 2 * n_heads)
        assert conv1d_weight.shape == (d_xBC, 1, d_conv)
        assert conv1d_bias.shape == (conv1d_weight.shape[0],)
        assert dt_bias.shape == (n_heads,)
        assert A.shape == (n_heads,)
        assert D_weight.shape == (n_heads, d_inner)
        ...
        if initial_states is not None:
            assert initial_states.shape == (2 * batch, n_heads, headdim, d_state)
        ...
        # Split of z, xBC and dt
        z, xBC_og, dt = torch.split(zxBCdt, [d_inner, d_xBC, 2 * n_heads], dim=-1)
        ...
        ctx.d_state = d_state
        ctx.headdim = headdim
        ctx.ngroups = n_groups

        return out

    @staticmethod
    @torch.amp.custom_bwd(device_type="cuda" if torch.cuda.is_available() else "cpu")
    def backward(ctx, dout, *args):
        ...
        dy = torch.cat([dy, torch.flip(dy, dims=(1,))], dim=0)
        dy[:, 0, :] = 0.0
